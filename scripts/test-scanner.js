...
/**
 * Test script for WhatsApp Scanner Library (JavaScript version for Termux)
 * 
...
// Import the built scanner (make sure to build first)
let WhatsAppScanner;
try {
  // Try to import from built version
  WhatsAppScanner = require('../packages/scanner/dist/index.js').WhatsAppScanner;
} catch (e) {
  // Fallback to source version
  WhatsAppScanner = require('../packages/scanner/src/index.ts').WhatsAppScanner;
}
...
async function testScanner() {
  ...
  try {
    ...
    log('5. Certifique-se de ter compilado o projeto: npm run build', colors.reset);
  }
}
